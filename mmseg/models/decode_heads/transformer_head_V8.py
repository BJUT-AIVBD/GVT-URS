...
class TransformerHeadV8(BaseDecodeHead):
    ...
    def __init__(self, pool_scales=(1, 2, 3, 6), **kwargs):
        super(TransformerHeadV8, self).__init__(
        ...
        for i in range(len(num_heads)):
            layer = TransformerSampleLayer(dim=self.channels, depth=depths[i], num_heads=num_heads[i],
                                           window_size=7,
    ...
    def forward(self, inputs):
        ...
        trs_outs = [
            self.upsamplayer[i](laterals[i])
            for i in range(used_backbone_levels - 1)
        ]

        trs_outs.append(laterals[-1])

        for i in range(used_backbone_levels - 1, 0, -1):
            trs_outs[i] = resize(
                trs_outs[i],
                size=trs_outs[0].shape[2:],
                mode='bilinear',
                align_corners=self.align_corners)

        trs_outs = torch.cat(trs_outs, dim=1)
        output = self.fpn_bottleneck(trs_outs)
        final_output = self.cls_seg(output)

        # heatmaps = draw_CAM(self, output, final_output, '/media/lws/Store/Dataset/unpaved_road_dataset/ALL_to_1200_800/test/image_V10/src/DSC03824.jpg', [0, 1])
        #
        # writer = SummaryWriter('../submits/Vis/Swin-S/transformer_head_V8/')
        # writer.add_images('test', heatmaps, 0)
        # writer.close()
