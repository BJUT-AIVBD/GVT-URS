...
class LePEAttention(nn.Module):
    def __init__(self, dim, resolution, idx, split_size=7, dilate=4, dim_out=None, num_heads=8, qkv_bias=False,
                 qk_scale=None,
        ...
        if idx == -1:
            ...
            H_sp, W_sp = self.resolution[0] // dilate, self.split_size
        elif idx == 1:
            W_sp, H_sp = self.resolution[1] // dilate, self.split_size
        else:
    ...
    def forward(self, temp, dilate=4):
        """
        x: B N C
        mask: B N N
        """
        ...
        if idx == -1:
            ...
            H_sp, W_sp = H // dilate, self.split_size
        elif idx == 1:
            H_sp, W_sp = self.split_size, W // dilate
        else:
...
class CSWinBlock(nn.Module):
    ...
    def __init__(self, dim, patches_resolution, num_heads,
                 split_size=7, dilate=4, mlp_ratio=4., qkv_bias=False, qk_scale=None,
    ...
    def forward(self, x):
        ...
        if self.branch_num == 2:
            ...
            x1 = self.attns[0](temp[:, :, :C // 2, idx_vrl, :], self.dilate)
            x2 = self.attns[1](temp[:, :, C // 2:, :, idx_hrl], self.dilate)
            attened_x = torch.cat([x1[:, idx_vrl_reverse, :, :], x2[:, :, idx_hrl_reverse, :]], dim=-1).reshape(B, -1, C)
        else:
            attened_x = self.attns[0](temp).reshape(B, -1, C)
...
class DCSWin(nn.Module):
    ...
    def __init__(self, img_size=[224, 224], patch_size=4, dilate=4, in_chans=3, embed_dim=64, depth=[1, 2, 21, 1],
                 split_size=7,
        ...
        self.stage1 = nn.ModuleList([
            CSWinBlock(
                dim=curr_dim, num_heads=heads[0], patches_resolution=[img_size[0] // 4, img_size[1] // 4],
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale, split_size=split_size[0], dilate=dilate,
        ...
        self.stage2 = nn.ModuleList(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[1], patches_resolution=[img_size[0] // 8, img_size[1] // 8],
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale, split_size=split_size[1], dilate=dilate,
                drop=drop_rate, attn_drop=attn_drop_rate,
        ...
        temp_stage3.extend(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[2], patches_resolution=[img_size[0] // 16, img_size[1] // 16],
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale, split_size=split_size[2], dilate=dilate,
                drop=drop_rate, attn_drop=attn_drop_rate,
        ...
        self.stage4 = nn.ModuleList(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[3], patches_resolution=[img_size[0] // 32, img_size[1] // 32],
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale, split_size=split_size[-1], dilate=dilate,
                drop=drop_rate, attn_drop=attn_drop_rate,
