...
class ConvBNPreLU(nn.Module):
    def __init__(self, in_channels, out_channels, kSize, stride, padding, dilation=(1, 1), groups=1, bn_act=False,
                 bias=False):
        super().__init__()
        self.bn_act = bn_act
...
class DSANet(nn.Module):
    def __init__(self, classes=11, block_1=3, block_2=4, block_3=4, frozen_stages=-1):
    ...
    def forward(self, input):
        ...
        return tuple([predict3, predict2, predict1, out])

    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(DSANet, self).train(mode)
        self._freeze_stages()
